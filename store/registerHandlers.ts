...
const handlers = {
    ...
    unpress: (() => { }) as ((x: number, y: number) => void),
};
...
export const registerUnpress = (unpress: (x: number, y: number) => void) => {
    handlers.unpress = unpress;
};
...
export const unpress = (x: number, y: number) => handlers.unpress(x, y);
